...
source_data = pd.read_csv('/data/sr/top.csv')
target_data = pd.read_csv('/data/sr/top2.csv')
...
model = Model(s1.shape[1], 1024,256).to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=8e-6, weight_decay=1e-6)
...
pd.DataFrame(tlabel_).to_csv('TCGA_Docetaxel_tlable.csv')
pd.DataFrame(tpred_).to_csv('TCGA_Docetaxel_tprde.csv')
...
plt.legend()
plt.title('features of TCGA_Docetaxel')
plt.savefig('TCGA_Docetaxel.jpg', dpi=500)
total_labels = total_labels.cpu().numpy()
total_feature = total_feature.numpy()
total_response = total_response.numpy()
pd.DataFrame(total_labels).to_csv('TCGA_Docetaxel_labels.csv')
pd.DataFrame(total_feature).to_csv('TCGA_Docetaxel_feature.csv')
pd.DataFrame(total_response).to_csv('TCGA_Docetaxel_response.csv')