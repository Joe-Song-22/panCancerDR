...
model1 = TestModel(s2.shape[1], 1024, 740).to(device)
...
#     print(label_)
    ...
    losses.append(loss_)
    model1.feature_extractor.load_state_dict(model.feature_extractor.state_dict())
    model1.classifier.load_state_dict(model.classifier.state_dict())
    with torch.no_grad():
        ...
        for i, data_t in enumerate(s_test):
            ...
            tc2, td3, trip, feature = model(t1_, tr_)
    ...
    if tauc > best_auc:
        best_auc = tauc
        torch.save(model1.state_dict(), '/data/sr/model1.pth')
    print(tauc)
...
# def model_predict(t1_, tr_):
#     model.eval()  # 确保模型处于评估模式
#     input_tensor = torch.tensor(t1_).float().to(device)  # 转为 PyTorch tensor
#     with torch.no_grad():
#         c2, t3, t_trip, feature = model(input_tensor, tr_)  # 只取模型的第一个输出
#         return feature.cpu().numpy()  # 返回 NumPy 数组
# t1_ = t1_.cpu().numpy()
# tr = tr.cpu().numpy()
# explainer = shap.KernelExplainer(model_predict, t1_, tr)
# shap_values = explainer.shap_values(t1_, tr)
# shap.summary_plot(shap_values, t1_)
pd.DataFrame(tlabel_).to_csv('PLX4720_tlable.csv')
pd.DataFrame(tpred_).to_csv('PLX470_tprde.csv')
pd.DataFrame(total_feature).to_csv('PLX4720_features.csv')
